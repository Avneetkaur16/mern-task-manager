...
const TaskComponent = ({ taskP }) => {
  ...
  const handleDelete = async() => {
    try {
      await axios.delete(`${process.env.REACT_APP_SERVER_URL}api/task/${taskP._id}`)
      navigate('/');
  ...
  const handleCheck = async(e) => {
    ...
    try {
      const { data } = await axios.put(`${process.env.REACT_APP_SERVER_URL}api/task/check/${taskP._id}`, { done: done ? false : true });
